...
class SearchDialog(wx.Dialog):
	def __init__(self, parent, session):
		super().__init__(parent, title=_("NVTube v5.0 - Volkan Ozdemir Software Services"), size=(450, 220))
		self.session = session
		panel = wx.Panel(self)
		vbox = wx.BoxSizer(wx.VERTICAL)
		lbl = wx.StaticText(panel, label=_("Search query:"))
	...
	def on_enter(self, event):
		...
		if query:
			speech.speakMessage(_("Searching for {query}").format(query=query))
			...
			with self.session.get(url, timeout=10, stream=True) as response:
				...
				for chunk in response.iter_content(chunk_size=40960, decode_unicode=True):
					...
					if match:
						...
						break
			if v_id:
				video_url = f"https://www.yout-ube.com/watch?v={v_id}&autoplay=1"
				webbrowser.open(str(video_url))
			else:
				ui.message(_("No results found."))
		except Exception as e:
			ui.message(_("An error occurred during search: {error}").format(error=str(e)))

class GlobalPlugin(globalPluginHandler.GlobalPlugin):
	def __init__(self):
		super().__init__()
		self.session = requests.Session()
		self.session.headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/120.0.0.0 Safari/537.36"}

	@scriptHandler.script(description=_("YouTube Search"), category=_("NVTube"))
